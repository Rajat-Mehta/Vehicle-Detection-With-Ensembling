...
def rescale_detections(ssd, shapes):
    ...
    for i, item in enumerate(ssd):
        ...
        for det in item:
            det_item = []
            det_item.append(det[0] / scales[i])
            det_item.append(det[1] / scales[i])
            det_item.append(det[2] / scales[i])
            det_item.append(det[3] / scales[i])
...
def write_csv_from_npy(path):
    ...
    if "ssd" in file_type or "frcnn" in file_type:
