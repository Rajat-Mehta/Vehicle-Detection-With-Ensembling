...
"""
This script can be used to train SSD model on our filtered dataset end-to-end.
All of the dataset paths have been configured in the arguments. You can start training by running this command:
python filtered_finetune_ssd_resnet.py
...
dataset = gcv.data.RecordFileDetection('../data_set_files/record_format_files/filtered_data_set/train.rec')
val_dataset = gcv.data.RecordFileDetection('../data_set_files/record_format_files/filtered_data_set/val.rec')

classes = ['bicycle','motorcycle','motorized_vehicle','pedestrian',
           'work_van','non-motorized_vehicle']  # only one foreground class here
...
train_data, val_data = get_dataloader(net, dataset, val_dataset, 512, 8, 0)
...
prefix = './train_logs/filtered_ssd_resnet/filtered_ssd_resnet'
# set up logger
logging.basicConfig()
...
for epoch in range(0, 100):
    ...
    net.save_parameters('./train_logs/filtered_ssd_resnet/epoch_'
                        +str(epoch)+'_ssd_512_resnet50_v1_voc_mio_tcd.params')